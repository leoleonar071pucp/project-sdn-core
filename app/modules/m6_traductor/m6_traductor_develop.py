...
class FlowBuilder:
    ...
    def t0_return_portal_flow(self, device_id, portal_ip):
        """T0 prio199 — respuesta del portal (IPV4_SRC=portal) → FLOOD.
        Permanente. Sin IN_PORT: el SYN-ACK llega por trunk port (variable).
        Usa FLOOD en lugar de NORMAL porque la FDB puede estar vacía al arrancar.
        Se instala en TODOS los switches para cubrir el camino de retorno completo."""
        return {
            "priority":    199,
            "isPermanent": True,
            "timeout":     0,
...
class M6Translator:
    ...
    def instalar_cuarentena_arranque(self):
        ...
        # ── T0: Table-miss NORMAL en TODOS los switches ───────────────────────
        # También en switches de acceso para que el tráfico en tránsito no
        # sea descartado al entrar por un trunk port sin flow específico.
        print(f"\n  [T0] Table-miss NORMAL (todos):")
        for device_id in devices:
            nombre = Config.SWITCH_NOMBRES.get(device_id, device_id[-4:])
        ...
        for device_id, puertos_acceso in access_switches.items():
            ...
            for puerto in puertos_acceso:
                flow = self.builder.ruta_directa_t0(
                    device_id, puerto, "dst", PORTAL_IP, "FLOOD",
                    prio=Config.PRIO_T0_PORTAL
                )
                self.onos.instalar_flow(device_id, flow)
            print(f"    ✓ {nombre} puertos={puertos_acceso}")

        # ── T0: Return flow portal en TODOS los switches ──────────────────────
        # El retorno del portal atraviesa switches de core y distribución,
        # no solo los de acceso. FLOOD cubre el camino completo sin depender
        # de la FDB que puede estar vacía al arrancar.
        print(f"\n  [T0] Return flow portal ({PORTAL_IP} → hosts, todos):")
        for device_id in devices:
            nombre = Config.SWITCH_NOMBRES.get(device_id, device_id[-4:])
