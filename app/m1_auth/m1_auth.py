...
def autenticar(codigo_pucp: str, password: str, ip_asignada: str) -> dict:
    ...
    # ── Verificar si el usuario ya tiene sesión activa en otro equipo ────
    sesion_existente = _sessions.get_session_by_usuario(id_usuario)
    if sesion_existente:
        mac_actual = sesion_existente.get("mac_address")
        ip_actual  = sesion_existente.get("ip_asignada")
        # Si la MAC del request es diferente a la MAC de la sesión activa
        # significa que está intentando loguearse desde otro equipo
        if Config.M6_HABILITADO:
            host = _tokens.resolver_host(ip_asignada)
            mac_request = host.get("mac") if host else None
        else:
            mac_request = Config.MAC_DUMMY

        if mac_request and mac_request != mac_actual:
            return {
                "ok": False,
                "motivo": f"Ya tienes una sesión activa desde otro equipo "
                          f"(IP: {ip_actual}). Cierra esa sesión primero.",
                "codigo_error": "SESION_YA_ACTIVA_OTRO_EQUIPO",
                "sesion_activa": {
                    "ip_asignada": ip_actual,
                    "mac_address": mac_actual,
                }
            }
